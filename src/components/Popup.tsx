...
export default function Popup() {
  ...
  // Throttle: limits how often search happens
  ...
  const debouncedSearch = debounce((q: string) => throttledSearch(q), 300);
  const throttledSearch = throttle((q: string) => runSearch(q), 200);
  ...
  return (
    ...
    [&::-webkit-scrollbar-thumb:hover]:bg-slate-400/70

    /* Smooth native style for Firefox */
    scrollbar-thin
  ...
  "
        >
          {filteredNotes.map((note, index) => {
            ...
            const linkMatch = note.link.toLowerCase().includes(q);
            return (
